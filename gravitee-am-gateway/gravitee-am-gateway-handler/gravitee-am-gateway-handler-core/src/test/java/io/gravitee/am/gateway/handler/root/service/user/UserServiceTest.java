...
public class UserServiceTest {
    ...
    public void mustSetEnrolSkipTime() {
        Client client = mock(Client.class);
        var skipEnrolSettings = new SkipEnrollSettings();
        skipEnrolSettings.setActive(true);
        skipEnrolSettings.setSkipTimeSeconds(7200L);
        var mfaSettings = new MFASettings();
        mfaSettings.setSkipEnroll(skipEnrolSettings);
        var user = mock(User.class);

        when(client.getMfaSettings()).thenReturn(mfaSettings);
        when(user.getEnrollSkippedAt()).thenReturn(null);

        doReturn(Single.just(user)).when(commonUserService).update(user);

        userService.setEnrolSkippedTime(client, user);
    ...
    public void mustNotSetEnrolSkipTime_settingsNotActive() {
        Client client = mock(Client.class);
        var skipEnrolSettings = new SkipEnrollSettings();
        skipEnrolSettings.setActive(false);
        skipEnrolSettings.setSkipTimeSeconds(7200L);
        var mfaSettings = new MFASettings();
        mfaSettings.setSkipEnroll(skipEnrolSettings);
        var user = mock(User.class);
    ...
    public void mustNotSetEnrolSkipTime_settingsUserIsNull() {
        ...
        userService.setEnrolSkippedTime(client, null);
    ...
    public void mustNotSetEnrolSkipTime_settingsUserExpiredAlreadySet() {
        ...
        var skipEnrolSettings = new SkipEnrollSettings();
        skipEnrolSettings.setActive(true);
        skipEnrolSettings.setSkipTimeSeconds(new Date(System.currentTimeMillis() + 86400L).getTime());
        var mfaSettings = new MFASettings();
        mfaSettings.setSkipEnroll(skipEnrolSettings);
        ...
        when(client.getMfaSettings()).thenReturn(mfaSettings);
        when(user.getEnrollSkippedAt()).thenReturn(new Date());
