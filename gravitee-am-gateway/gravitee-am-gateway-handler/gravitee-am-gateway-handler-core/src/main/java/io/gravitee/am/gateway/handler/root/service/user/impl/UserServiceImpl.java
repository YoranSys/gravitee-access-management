...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Completable setEnrolSkippedTime(Client client, User user) {
        var skipEnrolSettings = getSkipEnrolSettings(client);
        if (TRUE.equals(skipEnrolSettings.getActive()) && nonNull(user)) {
            Date now = new Date();
            long skipTime = ofNullable(skipEnrolSettings.getSkipTimeSeconds()).orElse(ConstantKeys.DEFAULT_ENROL_SKIP_TIME_SECONDS) * 1000L;
            if (isNull(user.getEnrollSkippedAt()) || user.getEnrollSkippedAt().getTime() + skipTime < now.getTime()) {
                user.setEnrollSkippedAt(now);
    ...
    private SkipEnrollSettings getSkipEnrolSettings(Client client) {
        return ofNullable(client)
                .filter(Objects::nonNull)
                .map(Client::getMfaSettings)
                .filter(Objects::nonNull).map(MFASettings::getSkipEnroll)
                .orElse(new SkipEnrollSettings());
    }
